...
function anime:drawAnime(name) --绘制动画（若不播放，将绘制当前帧）
	...
	if list then
		...
		if list.nowPos < 1 then
			list.nowPos = list.num
		end
		if list.nowPos > list.num then
			list.nowPos = 1
		end
	else
...
function anime:setAnimePos(name) --指定播放位置
	
	local list = animeList[name]

	if list then
		return {
			afterPos = function() --播放上一帧
				list.nowPos = list.nowPos - 1
			...
			setPos = function(pos) --设置播放位置
				list.nowPos = pos

				if list.nowPos < 1 or list.nowPos > list.num then
					error("'"..name.."' this is anime of pos '"..pos.."' is not find.")
				end
			end
		}
	else
...
function anime:setAnimeAxis(name,...) --设置动画位置
	...
	animeList[name].axis.arc = axis[3] or animeList[name].axis.arc
